...
class TeacherHomeworkAdapter(layoutResId: Int, data: List<TeacherHomeworkBean>?,val type:Int) : BaseQuickAdapter<TeacherHomeworkBean, BaseViewHolder>(layoutResId, data) {
    ...
    override fun convert(helper: BaseViewHolder, item: TeacherHomeworkBean) {
        helper.apply {
            setText(R.id.tv_status,"${item.subject}    ${when (item.status){ 1-> "通知" 2-> "提交" else ->"批改"}}")
            ...
            setText(R.id.tv_commitTime,if (item.status==1)"" else "学生提交时间："+DateUtils.longToStringWeek2(item.time))
            setText(R.id.tv_startTime, "布置时间："+DateUtils.longToStringWeek2(DateUtils.dateStrToLong(item.createTime)))
